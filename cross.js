...
mongoose.connect('mongodb+srv://emo31:alihan935@cluster0.dzfil.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', { useNewUrlParser: true, useUnifiedTopology: true }); // Mongo connect bağlantısı.
const Database = require("./models/role.js");
// Cross tarafından kodlanmıştır.
let arr = ayarlar.tokens
let tokens = []

for (data of arr) {
    const ayarlar = require('./ayarlar.json');
    let bot = new Client({ fetchAllMembers: true });
    bot.login(data).then(c => console.log(`${bot.user.tag} olarak giriş yapıldı!`)).catch(err => console.error("Bota giriş yapılırken başarısız olundu!"));
    tokens.push(bot)
    bot.on("ready", async() => {
        bot.user.setPresence({ activity: { name: "Cross was here!" }, status: "idle" });

    });
}

client.on("ready", async() => {
    client.user.setPresence({ activity: { name: "Cross was here!" }, status: "idle" });
...
client.on("message", async message => {
    ...
    let embed = new MessageEmbed().setColor("#00ffdd").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Cross"} was here!`).setTimestamp();
    ...
    if (command === "safe") {
        if (message.author.id !== "799237103195127848" && message.author.id !== "733138751487869069") return message.channel.send(new MessageEmbed().setColor('RANDOM').setFooter('Cross was here!').setDescription(`Bu Komutu Kullanamazsınız.`))
        let hedef;
        ...
        if (guvenliler.some(g => g.includes(hedef.id))) {
            ...
            message.channel.send(embed.setDescription(`${hedef}, ${message.author} tarafından güvenli listeden atildi :grin::grin:!`));
        } else {
            ayarlar.whitelist.push(`y${hedef.id}`);
            fs.writeFile("./ayarlar.json", JSON.stringify(ayarlar), (err) => {
                if (err) console.log(err);
            });
            message.channel.send(embed.setDescription(`${hedef}, ${message.author} tarafından koruma altina alindi :grin:`));
        };
    };
    ...
    if (command === "kur" || command === "kurulum" || command === "backup" || command === "setup") {
        ...
        Database.findOne({ guildID: ayarlar.guildID, roleID: args[0] }, async(err, roleData) => {
            if (!roleData) return message.channel.send(embed.setDescription("Düzgün ID Gir mal oç :grin:"));
            message.react("✅");
            ...
            async function rolver(kisiID, tur) {
                let uye1 = tokens[0].guilds.cache.get(ayarlar.guildID).members.cache.get(kisiID);
                let uye2 = tokens[1].guilds.cache.get(ayarlar.guildID).members.cache.get(kisiID);
                ...
                uye1.roles.add(yeniRol.id).catch(e => {});
                uye2.roles.add(yeniRol.id).catch(e => {});
            ...
            if (logKanali) { logKanali.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`**${roleData.name} (${roleData.roleID})** rolünü silmiş orspular aciom hemen sakin olun arkdslr :grin:`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Cross"} was here!`).setTimestamp()).catch(); } else { message.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Cross"} was here!`).setTimestamp()).catch(err => {}); };
        });
    };
});
...
function cezalandir(kisiID, tur) {
    let uye = client.guilds.cache.get(ayarlar.guildID).members.cache.get(kisiID);
    if (!uye) return;
    if (tur == "ban") return uye.ban({ reason: "Ayağı Kaydi" }).catch();
};
...
function setRoleBackup() {
    ...
    if (guild) {
        ...
        console.log(`Başarılı bir şekilde rol backup alındı!`);
    };
};
// Yt kapat fonksiyonu
...
client.on("message", async(message) => {
    if (message.content === "sa") {
        message.guild.channels.create('31', {
            type: 'voice'
